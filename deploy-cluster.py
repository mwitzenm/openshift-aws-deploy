...
@click.option('--region', default='us-east-1', help='ec2 region',
              show_default=True)
@click.option('--availability_zone', default='us-east-1a', help='ec2 availability zone',
              show_default=True)
...
def launch_refarch_env(region=None,
  ...
  click.echo('\tavailability_zone: %s' % availability_zone)
  ...
  # Run playbooks
  playbooks = ['playbooks/infrastructure-setup.yaml', 'playbooks/openshift-install.yaml']
  for playbook in playbooks:
    ...
    # Refresh the inventory cache to prevent stale hosts from interferring with re-running
